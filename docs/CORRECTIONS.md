...
**Stage 2 of `wp1_mr_pipeline.py` was never executable, and the L2 status note
above overstates what the module commits.**

The `run` subcommand is a stub. It prints the "not validated end to end" notice
and returns exit code 2. It never loads an outcome file, never harmonises,
never iterates over taxa and never writes output. The statement in the L2
status section that the module "commits the full method: instrument selection,
harmonisation, IVW, MR-Egger, weighted median, Cochran Q and BH-FDR" is
inaccurate with respect to harmonisation, which is not committed anywhere in
the module.

Found by RLO_MR_OUTCOME-STAGE2-VALIDATION_v1.0 on 2026-08-07. Stage 2 was
executed by importing the committed estimator functions unchanged into
`src/wp1_stage2_validate.py`, which supplies the missing harmonisation and per
taxon loop. The committed pipeline was not modified.

## C14

**Clumping window disagreement. The pipeline uses 500 kb; the committed result
tables were generated with 1 Mb.**

`CLUMP_WINDOW = 500_000` in `wp1_mr_pipeline.py`. Rebuilding at 500 kb
reproduces the committed `n_snp` for 70 of 211 taxa in the colorectal outcome.
Rebuilding at 1 Mb, with C15 also corrected, reproduces 208 of 211.

## C15

**Palindromic SNPs are removed at the wrong stage. The pipeline removes them
before clumping; the committed tables were generated by removing them during
harmonisation, after clumping.**

Order matters because clumping is greedy on p value. Removing a palindromic SNP
before clumping allows a different, more distant SNP to win the clump. Isolated
by toggling this alone: reproduction of `n_snp` rises from 76 of 211 to 205 of
211 when the drop is moved after clumping, holding the window at 1 Mb.

## C16

**MR-Egger is not oriented. The committed `mr_egger` omits the step that orients
exposure effects positive.**

Without that step the regression slope and intercept are not the quantities
MR-Egger defines, and the reported `egger_intercept_p` is not the Egger
intercept pleiotropy test. Reproduction of `p_egger` under the committed
implementation is 1 to 2 taxa out of 211 across the three outcomes, consistent
with chance. With orientation restored it is 207 to 209 of 211.

Every MR-Egger and pleiotropy figure reported from this pipeline is affected.
No claim is edited by this correction; see C13 through C20 status note below.

## C17

**IVW standard error is fixed effect in the pipeline; the committed tables use
random effects.**

The committed `ivw` returns `se = sqrt(1 / sum(w * bx^2))` with no
overdispersion scaling. The tables are reproduced only when the standard error
is scaled by `sqrt(max(1, Q / df))`.

The signature that isolated this: for taxa where the instrument set happened to
match, `b_ivw` and Cochran Q reproduced to the last digit while `p_ivw` did
not. The IVW point estimate and the Cochran Q statistic in the committed
pipeline are correct. Only the standard error, and therefore every `p_ivw` and
every confidence interval under heterogeneity, is affected. Reproduction of
`p_ivw` rises from 88 of 211 to 205 of 211 on this change alone.

## C18

**Multi allelic rsID row selection is undocumented and outcome file order
dependent.**

FinnGen carries more than one row per rsID at multi allelic sites. Whether an
instrument harmonises or is discarded as an allele mismatch depends on which
row is retained. Confirmed sites in the colorectal outcome:

| rsID | Exposure | Resolving row | Mismatching row |
|---|---|---|---|
| rs10805326 | G/A | G/A | G/T |
| rs9580476 | C/T | T/C | T/G |
| rs2482038 | C/A | A/C | A/T |
| rs2435610 | A/C | C/A | C/T |

Retaining the last row leaves six taxa one instrument short in both FinnGen
outcomes. Retaining the first row recovers them. The committed tables are
consistent with first row retention. The correct behaviour is to select the row
whose allele pair matches the exposure, not to depend on file order. Neither
rule is documented in the pipeline.

## C19

**Instruments with rsID `NA` are selected and then silently discarded.**

`MBG.allHits.p1e4.txt` contains rows whose `rsID` field is the literal string
`NA`. `select_instruments` applies no rsID validity filter, so such rows are
eligible to be selected, counted toward the instrument set, and then lost at
harmonisation with no warning. Confirmed for `order.Clostridiales.id.1863` and
`class.Clostridia.id.1859`, each carrying one such instrument.

## C20

**The three committed result tables use three different schemas, and the
colorectal table records effects only as a rounded display string.**

| Table | Effect columns |
|---|---|
| `MR_results_colorectal.tsv` | `OR (95% CI)` as a string. No `b`, no `se` |
| `MR_results_colorectal_100k.tsv` | `OR, b, se` |
| `MR_results_pancreatic.tsv` | `or_ivw, b_ivw, se_ivw` |

This is the residual of C7. Full precision reproduction of the colorectal table
cannot be checked, because the original floats do not exist anywhere in the
repository and cannot be recovered from `0.79 (0.71-0.89)`. The other two
tables were checked at float level and reproduce for 209 of 211 and 208 of 211
taxa respectively under the corrected implementation.

---

### Status note for C13 through C20

These corrections record defects in the committed pipeline and in the committed
table schemas. They do not, by themselves, establish that any published result
is wrong. The faithful reconstruction reproduces the committed numbers for 207
to 209 of 211 taxa per outcome, including full precision `b` and `se` where
those exist, which indicates the tables are internally consistent with a
...
committed pipeline is not that implementation.

No README, manuscript, poster or figure claim was modified under
RLO_MR_OUTCOME-STAGE2-VALIDATION_v1.0, per its Section 12. Evidence is in
`docs/findings/MR_PIPELINE_VALIDATION.md`.

### Status of L2, updated 2026-08-07

Stage 1 remains validated. Stage 2 is now EXECUTED against real outcome data
