...
| L2 | **Partially closed, and the committed pipeline does not reproduce the published tables.** Stage 1, instrument selection, is validated against all 211 committed counts (0 deficits, median ratio 1.11). Stage 2 was executed against real outcome data by a separate harness, `src/wp1_stage2_validate.py`: a faithful reconstruction reproduces the published `b`, `se` and `p` for 209/211 taxa at float precision, while `wp1_mr_pipeline.py`'s own parameters reproduce only 20/211. Six parameter and estimator discrepancies are diagnosed as [C14-C19](docs/CORRECTIONS.md). **No published claim changes**: a flag audit found 0/211 Figure 6 classifications altered. Note also [C13](docs/CORRECTIONS.md): `wp1_mr_pipeline.py run` is a stub and an earlier README claim that harmonisation was implemented was false |
